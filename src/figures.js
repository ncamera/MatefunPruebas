...
const truncatedConeGeometry = new THREE.CylinderGeometry(topRadius, bottomRadius, distanceBetweenCircles, 64);
 ...
 */
function triangulatePolygon(polygon2D, height) {
    const facesOfPolygon2D = [];
    const indicesOfFacesOfPolygon = new Delaunator(polygon2D)
        .triangles;
    indicesOfFacesOfPolygon.forEach((entry) => {
        facesOfPolygon2D.push(polygon2D[entry * 2], polygon2D[entry * 2 + 1], height);
    });
...
function graphPolygon(triangles) {
...
function addWireframeToFigure(figure) {
...
function addWireframeTo3DPolygon(figure) {
 ...
 */
function complete3DFigureWithFaces(topPolygon, bottomPolygon, topPolygonHeight, bottomPolygonHeight) {
...
const wireframeMesh2 = new THREE.LineSegments(geomentryEdgesMesh2, materialMesh2);
...
const geometryLineConeHeight = new THREE.BufferGeometry().setFromPoints(pointsLineConeHeight);
